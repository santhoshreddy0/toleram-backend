...
router.patch("/:teamId", verifyRole("admin"), async (req, res) => {
  ...
  router.post("/:teamId/addPlayer", verifyRole("admin"), async (req, res) => {
    const { teamId } = req.params;
    ...
    try {
      ...
      if (playerRows.length > 0) {
        return res.status(400).json({ message: "Player with the name already exists" });
      }
